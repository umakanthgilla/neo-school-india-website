...
r=await call('school/assignments/'+schoolId,'POST',{request_id:'WRONGCLASS',route_id:'ROUTE001',student_id:'CHILD001',classroom_id:'CLASS002',stop:'Market'});assert(r.status===400,'wrong section must fail '+JSON.stringify(r));
r=await call('school/assignments/'+schoolId,'POST',{request_id:'ASSIGN001',route_id:'ROUTE001',student_id:'CHILD001',classroom_id:'CLASS001',stop:'Market'});assert(r.status===201,'assign child '+JSON.stringify(r));
r=await call('school/assignments/'+schoolId,'POST',{request_id:'ASSIGN003',route_id:'ROUTE001',student_id:'CHILD001',classroom_id:'CLASS001',stop:'School'});assert(r.status===409,'second child assignment must fail '+JSON.stringify(r));
r=await call('school/assignments/'+schoolId+'/ASSIGN001','PATCH',{active:false});assert(r.status===403,'locked assignment cannot be removed '+JSON.stringify(r));
r=await call('school/routes/'+schoolId+'/ROUTE001','PATCH',{active:false});assert(r.status===409,'route with assigned child cannot close '+JSON.stringify(r));
r=await call('school/assignments/'+schoolId,'POST',{request_id:'ASSIGN002',route_id:'ROUTE001',student_id:'CHILD002',classroom_id:'CLASS002',stop:'Market'});assert(r.status===409,'capacity must fail '+JSON.stringify(r));
