 ...
 async function school(area,{school,token,base}){
  ...
  async function draw(){
   try{
    ...
    const vehicles=d.vehicles.filter(v=>v.active),routes=d.routes.filter(r=>r.active),assignments=d.assignments.filter(a=>a.active),trips=d.trips.filter(t=>t.date===today);
    ...
    area.innerHTML='<div class="learning-welcome"><div><span class="eyebrow">SCHOOL OPERATIONS · TRANSPORT</span><h2>Transport</h2><p>Manage assigned children and record today’s pickup and drop. GPS is not required for this stage.</p></div></div><p class="portal-status" data-transport-status role="status"></p>'+
     ...
     form('Child assignment',select('route_id','Route',option(routes,r=>r.name))+select('student_id','Student',option(d.students,s=>s.name+' · '+s.program))+'<label>Pickup / drop stop<select name="stop" required><option value="">Select route first</option></select></label>','assignments')+
     form('Today’s trip',select('route_id','Route',option(routes,r=>r.name))+'<label>Direction<select name="direction" required><option value="Pickup">Morning pickup</option><option value="Drop">Return drop</option></select></label>','trips')+'</div>'+
     '<section class="panel"><h3>Routes & assigned children</h3>'+ (routes.length?routes.map(r=>'<div class="transport-row"><b>'+esc(r.name)+'</b> · '+esc(d.vehicles.find(v=>v.id===r.vehicle_id)?.registration_no||'')+' · '+esc(d.staff.find(s=>s.id===r.driver_staff_id)?.name||'')+'<p>Stops: '+esc(r.stops.join(' → '))+'</p><p>Children: '+esc(assignments.filter(a=>a.route_id===r.id).map(a=>(names.get(a.student_id)||a.student_id)+' ('+a.stop+')').join(', ')||'None')+'</p><button class="secondary" data-disable="routes" data-id="'+esc(r.id)+'">Close route</button></div>').join(''):'<p>No routes yet.</p>')+'</section>'+
     '<section class="panel"><h3>Today’s trips</h3>'+ (trips.length?trips.map(t=>{const r=routes.find(x=>x.id===t.route_id);return '<div class="transport-row"><b>'+esc(r?.name||t.route_id)+' · '+esc(t.direction)+'</b><p>Status: '+esc(t.status)+' · '+(t.events||[]).length+' child events</p><div class="actions">'+(['Started','Approaching','At stop','Arrived school','Completed'].filter(s=>({Planned:['Started'],Started:['Approaching','At stop','Arrived school','Completed'],Approaching:['At stop','Arrived school','Completed'],'At stop':['Approaching','Arrived school','Completed'],'Arrived school':['Completed']}[t.status]||[]).includes(s)).map(s=>'<button class="secondary" data-trip="'+esc(t.id)+'" data-next="'+s+'">'+s+'</button>').join(''))+'</div><div class="portal-grid">'+assignments.filter(a=>a.route_id===t.route_id).map(a=>'<label>'+esc(names.get(a.student_id)||a.student_id)+'<select data-trip-child="'+esc(t.id)+'" data-student="'+esc(a.student_id)+'"><option value="">Record child event…</option>'+(t.direction==='Pickup'?['Ready','Picked up','Dropped at school','Absent']:['Boarded at school','Dropped at stop','Absent']).map(s=>'<option>'+s+'</option>').join('')+'</select></label>').join('')+'</div></div>'}).join(''):'<p>No trip started for today.</p>')+'</section>';
