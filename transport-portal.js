 ...
 async function school(area,{school,token,base}){
  ...
  async function draw(){
   try{
    ...
    const vehicles=d.vehicles.filter(v=>v.active),routes=d.routes.filter(r=>r.active),assignments=d.assignments.filter(a=>a.active),trips=d.trips.filter(t=>t.date===today),availableStudents=d.students.filter(s=>!d.assignments.some(a=>a.student_id===s.id)),classrooms=d.classrooms||[],programs=[...new Set(classrooms.map(c=>c.program))];
    ...
    area.innerHTML='<div class="learning-welcome"><div><span class="eyebrow">SCHOOL OPERATIONS · TRANSPORT</span><h2>Transport</h2><p>Manage assigned children and record today’s pickup and drop. GPS is not required for this stage.</p></div></div><p class="portal-status" data-transport-status role="status"></p>'+
     ...
     form('Child assignment',select('route_id','Route',option(routes,r=>r.name))+'<label>Class<select name="program" required><option value="">Choose class…</option>'+programs.map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('')+'</select></label><label>Section<select name="classroom_id" required><option value="">Select class first</option></select></label><label>Student<select name="student_id" required><option value="">Select section first</option></select></label><label>Pickup / drop stop<select name="stop" required><option value="">Select route first</option></select></label><p>Choose Class → Section → Student. Only students in that section without a transport assignment appear. Once saved, the child’s route and stop are locked.</p>','assignments')+
     ...
     '<section class="panel"><h3>Routes & assigned children</h3>'+ (routes.length?routes.map(r=>'<div class="transport-row"><b>'+esc(r.name)+'</b> · '+esc(d.vehicles.find(v=>v.id===r.vehicle_id)?.registration_no||'')+' · '+esc(d.staff.find(s=>s.id===r.driver_staff_id)?.name||'')+'<p>Stops: '+esc(r.stops.join(' → '))+'</p><p>Children: '+esc(assignments.filter(a=>a.route_id===r.id).map(a=>(names.get(a.student_id)||a.student_id)+' ('+a.stop+')').join(', ')||'None')+'</p>'+(assignments.some(a=>a.route_id===r.id)?'<small>Assigned children are locked to this route.</small>':'<button class="secondary" data-disable="routes" data-id="'+esc(r.id)+'">Close empty route</button>')+'</div>').join(''):'<p>No routes yet.</p>')+'</section>'+
    ...
    const assignmentForm=area.querySelector('form[data-kind="assignments"]'),programSelect=assignmentForm.querySelector('[name="program"]'),sectionSelect=assignmentForm.querySelector('[name="classroom_id"]'),studentSelect=assignmentForm.querySelector('[name="student_id"]');
    programSelect.onchange=()=>{sectionSelect.innerHTML='<option value="">Choose section…</option>'+classrooms.filter(c=>c.program===programSelect.value).map(c=>'<option value="'+esc(c.id)+'">'+esc(c.name)+' · '+esc(c.academic_year||'')+'</option>').join('');studentSelect.innerHTML='<option value="">Select section first</option>'};
    sectionSelect.onchange=()=>{studentSelect.innerHTML='<option value="">Choose student…</option>'+option(availableStudents.filter(s=>s.classroom_id===sectionSelect.value),s=>s.name)};
    ...
    area.querySelectorAll('[data-disable]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await api(b.dataset.disable,'PATCH',{active:false},b.dataset.id);await draw()}catch(err){msg(area,err.message);b.disabled=false}});
